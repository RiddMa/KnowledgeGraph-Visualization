...
\buptfigure[height=0.35\textheight]{pictures/CyKG_Class-Backend.png}{后端服务子系统类图}{CyKG_Class-Backend}

\subsubsection{功能时序图}

图\ref{CyKG_Sequence-UserSequence}所示为通用状态下用户操作前端发起请求的时序图。

\buptfigure[width=0.8\textwidth]{pictures/CyKG_Sequence-UserSequence.png}{用户请求功能时序图}{CyKG_Sequence-UserSequence}
...
\subsubsection{知识图谱可视化数据 API 实现 /api/graph/<limit>}
...
\buptfigure[width=0.8\textwidth]{pictures/Impl_Backend.png}{知识图谱搜索 API 流程图}{pictures/Impl_Backend.png}

\ref{pictures/Impl_Backend.png}采用模块化设计，
...
搜索视图包含一个悬浮搜索框，显示在容器的左上角。在搜索框输入关键词并按下回车，发出异步 I/O 请求并等待返回绘图数据。可视化显示流程与上述章节 \ref{可视化视图实现} \nameref{可视化视图实现}相似。
...
\buptfigure[width=\textwidth]{pictures/Dashboard_Waterfall.png}{概览视图加载请求瀑布流}{pictures/Dashboard_Waterfall.png}
